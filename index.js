  ...
  try {
    ...
    res
      .cookie('access_token', token, {
        ...
        secure: process.env.NODE_ENV === 'production', // la cookie solo se puede acceder en https en producción
...
app.get('/protected', (req, res) => {
  const { user } = req.session
  if (!user) return res.status(403).send('Access token not authorized')
